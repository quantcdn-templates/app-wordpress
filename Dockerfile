...
RUN curl -O https://raw.githubusercontent.com/wp-cli/wp-cli/v2.11.0/utils/wp-completion.bash \
    ...
    && \
    # Create fallback wp-env.sh file (entrypoint will update with runtime values)
    cat > /tmp/wp-env.sh << 'EOF'
# Fallback environment variables for wp-cli
# This ensures wp-cli works even if entrypoints are overridden
# The entrypoint script will update this with actual runtime values
